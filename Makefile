...
csapp.o: csapp.c cache.c csapp.h
	$(CC) $(CFLAGS) -c csapp.c
...
handin:
	(make clean; cd ..; tar cvf proxylab-handin.tar proxylab-handout --exclude "*.o" --exclude tiny --exclude nop-server.py --exclude proxy --exclude driver.sh --exclude port-for-user.pl --exclude free-port.sh --exclude ".*")
